...
if (!isMainThread) {
  ...
  if (DEBUG)
    process.stderr.write(
      `[jude-map worker] isMainThread=false command=${command}\n`,
    );

  if (DEBUG)
    process.stderr.write(
      `[jude-map worker] calling fromSharedBuffer segSab.byteLength=${segSab.byteLength} maxInputBytes=${maxInputBytes}\n`,
    );
  const seg = SharedTensorSegment.fromSharedBuffer(segSab, maxInputBytes);
  if (DEBUG)
    process.stderr.write(
      `[jude-map worker] fromSharedBuffer done, seg.byteCapacity=${seg.byteCapacity}\n`,
    );
  ...
  switch (command) {
    ...
    case "read": {
      if (DEBUG)
        process.stderr.write(`[jude-map worker] entering case "${command}"\n`);
      ...
      if (DEBUG) process.stderr.write(`[jude-map worker] postMessage sent\n`);
      if (DEBUG)
        process.stderr.write(
          `[jude-map worker] case "${command}" done, posting message\n`,
        );
